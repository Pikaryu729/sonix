...
**Resource-exhaustion defense:** header and body size limits are enforced *during accumulation*, not after the full request has been buffered. "Buffer everything, then check" is the vulnerability, not a mitigation of it. Two subtler versions of the same idea, both found by the review pass rather than by design:

- **A numeric field is capped by digit count before it is converted.** `Content-Length: 000…0` with a few hundred thousand digits is a well-formed header under `max_header_size`, but CPython refuses to build an `int` from more than 4300 digits and raises a plain `ValueError` — which is not an `HTTPParserError`, so it escaped the bridge's handler and killed the connection instead of answering 400. Anything longer than 20 digits cannot be a length this server would accept anyway (2⁶⁴ is 20 digits), so the cap costs nothing and turns the crash into the 400 it should always have been. The chunk-size token gets the same treatment.
- **Chunked trailers count against `max_headers`.** Trailers are headers; parsing them under a separate code path meant they were bounded only by `max_header_size`, so a request could carry tens of thousands of them past a `max_headers` of 100. The limit now spans both, because it is a limit on a request, not on a section of one.
...
- **Pipelining depth is bounded, by the same mechanism as body backpressure.** Nothing limited how many pipelined requests a single `data_received` could dispatch: the parser emits every complete request in the buffer before returning, so one modest write could spawn thousands of concurrent tasks, each with its own turnstile event and receive queue. Reading pauses once the in-flight count reaches `pipeline_pause_watermark` and resumes below `pipeline_resume_watermark` — the transport's own read pause, shared with the body-chunk path, so the two never fight over `resume_reading()`. The requests already parsed out of the buffer still run; the cap is on how far ahead of the responses the *reading* may get.
...
10. 🟡 **Hardening pass** — the connection half is done: a re-armable keep-alive/idle timeout (the old one covered only the first request head, and re-arming stays conditional on the connection still being in HTTP mode), `set_write_buffer_limits()` plus a fix for a single-slot `_drain` that could hang a websocket application with two sender tasks, `Date`/`Server` response headers, and deferring a websocket close until the application task has drained — which took Autobahn from 11 `NON-STRICT` to 4. What remains: the `wrk` benchmark against FastAPI+uvicorn, and fail-fast incremental UTF-8 validation (the 4 remaining `NON-STRICT` cases). Deliberately still open, not gaps: `permessage-deflate` (an explicit non-goal), the O(n) buffer front-deletion both parsers share, and — newly named rather than newly true — **no timeout of any kind while a task is in flight**. The two timer modes cover the two idle shapes (awaiting a first head, and idle between keep-alive requests); once a handler task exists, `_arm_idle_if_free` declines to arm and nothing bounds the connection in time again until that task finishes. Two consequences fall out of that one sentence, and it is worth writing them as one gap rather than two: a declared `Content-Length` dripped a byte per second is bounded in volume by `max_body_size` but not in time, and a head already pipelined behind a slow handler sits unbounded for the same reason. Closing it means a timer whose deadline is a *rate* rather than a state — a different design from the current mode switch, which is why it is deferred rather than patched.
...
Note that several step-10 hardening items arrived early, because the protocol layer couldn't be written correctly without them: header/body size limits, the slow-loris head timeout, and read/write backpressure watermarks are already in place. The review passes have since run too — `code-reviewer` and `security-reviewer` over `server/**`, which found an uncaught `ValueError` from an oversized `Content-Length` digit string, unbounded pipelining depth, and chunked trailers escaping `max_headers`. What remains for step 10 is the benchmark.
